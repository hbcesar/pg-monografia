...
\banca{Muito interessante este parágrafo. Só no final, última linha, quando fala "principalmente na produção de modelos". Essa palavra 'modelos' ficou solta. Que modelos, quaisquer modelos? Pode encadear melhor essa aparição da palavra modelos com o que foi dito na oração anterior (modelos que auxiliam a entender...). Talvez só usar uma palavra para referenciar como "principalmente na produção de tais modelos".}
...
\banca{achei pouco 'formal' esse "além do que".
	se quiser, pode utilizar somente "além, o processo de modelagem..." ou então "ainda, o processo de modelagem..."}
...
\banca{enfatizar neste primeiro parágrafo que o Eclipse é uma plataforma de desenvolvimento orientado a modelos}
...
\banca{acho que pode ficar mais claro que a EMF trabalha somente com modelos Ecore (ou pelo menos de forma nativa). "por meio de modelos Ecore"}
...
\banca{A exemplificação que você deu na apresentação transformação em Java seria uma bela adição neste texto complementando a ilustração de como o Eclipse apoia todo o processo de MDD, desde o modelo conceitual até o código.}
...
\banca{Muitas citações indiretas ao mesmo trabalho. Como não são citações diretas, com a transcrição completa do conteúdo, acho que não precisa fazer a citação indireta tantas vezes. Neste primeiro parágrafo sobre o Sirius, por exemplo, talvez possa ser citado indiretamente o trabalho Viyovic, Maksimovic, Perisic apenas uma vez no final.}
\banca{"arquivos formato .odesign" ou "arquivos do tipo .odesign"}
\banca{parece que se fala de dois metamodelos neste parágrafo: metamodelo do editor e metamodelo do domínio de interesse. se tiver como deixar essa diferença, esses dois metamodelos mais claro é bom.}
...
\banca{Achei estranha a definição. O que o sistema pode prover? O que o sistema é capaz de fazer? Como os requisitos são elicitados antes, no início do processo de engenharia de software, eles são na verdade uma descrição do que o sistema "deverá prover" quando estiver pronto.}
...
\banca{Dar um exemplo de como esse processo é realizado.}
...
\banca{Qual versão do istar que vc está usando? A versão original não tem quality constraints, nem domain assumptions.}
...
% ======================================================================================================
\subsection{GORO}
\banca{Onde está o modelo conceitual da ontologia?}
...
\goro foca principalmente no conceito de objetivos, tendo como principal meta esclarecer e explorar a natureza dos conceitos envolvidos, buscando assim aumentar a eficiência da comunicação entre os \textit{stakeholders} de um projeto. 
...
\banca{Como assim enviar mensagens? Que mensagens? Talvez dizer algo mais genérico(para que servem essas mensagens)? "o sistema passa a monitorar/interagir com essas instâncias ..."}
...
\begin{figure}
	...
	\caption{Metamodelo que define a sintaxe abstrata para o \zanshin. \cesar{modificar figura}}
...
\banca{acho importante citar os quatro componentes, mesmo os que não serão tratados neste trabalho. ajuda a dar uma visão geral do framework.}
...
\begin{table}
	...
	\begin{tabular}{|l|l|l|}
		\hline
		AR15 & NeverFail(G\_RegCall) & \begin{tabular}[c]{@{}l@{}}1. Retry(5000)\\ 2. RelaxDisableChild(T\_DetectCaller)\end{tabular} \\ \hline
	\end{tabular}
\end{table}
